...
function InformUser(game, user, message, colour = "#FFFF00", thumb = "") {
    ...
    if (thumb != "") {
        embed.setThumbnail("https://vicr123.github.io/ffc/" + thumb);
    }
...
exp.prototype.ParseDM = function(user, message, messageObj) {
    ...
    if (this.users.indexOf(user) == -1 && this.spectators.indexOf(user) == -1) {
        ...
        if (lmessage == "hand" && !spectator) {
            ...
            if (this.finished) {
                ...
                if (this.resolvingPlusFour) {
                    ...
                    if (this.HasPlayableCards(user, true)) {
                        ...
                        embed.setThumbnail("https://vicr123.github.io/ffc/drawCard.png");
                        user.send("", {embed: embed});
            ...
            if (this.challengeUser != null) {
                //Resolve User Challenge first
                this.challengeUser = null;
                user.send("yay challenge");
            } else if (this.resolvingPlusFour) {
                if (this.plusFourChallengeSuccess) {
                    this.IterateUsers(function(me, iuser) {
                        InformUser(me, iuser, user.username + " has challenged the Wild Draw Four played by " + me.plusFourPlayer.username + ", and has won the challenge. " + me.plusFourPlayer.username + " has drawn four cards.", "#FFFF00", "challengeSuccess.png");
                    });
                    ...
                    embed.setThumbnail("https://vicr123.github.io/ffc/challengeFail.png")
                    embed.setColor("#FF0000");
                    this.IterateUsers(function(me, iuser) {
            ...
            if (this.houseRules.cumulativeDraw) {
                ...
                if (hasDrawTwo) {
                    if (this.userDecks[user.id].length == 0) {
                        ...
                        this.IterateUsers(function(me, iuser) {
                            InformUser(me, iuser, user.username + " has forgotten to call FFC! Challenge this with the `!` command now!");
                        });
                        
                        let me = this;
                        process.nextTick(function() {
                            me.challengeUser = user.id;
                        });
                    }
